...
target_img = cv.imread('Camera Roll/Ali Tabasi/69.jpg', 0)
...
fnames = glob.glob('Camera Roll/Ali Tabasi/*.jpg')
fnames.sort()
...
while rval:
    ...
    rval, base_img = vc.read()
    base_img = cv.resize(base_img , (1280,720))
    # print(rval)
    i += 1
    if i % 3 != 1:
        pass
    else:
        # base_img = cv.imread('Camera Roll/Ali Tabasi/69.jpg', 0)
        dets4 = detector(base_img, 1)
        Ali=mean1.copy()
        for k, d in enumerate(dets4):
            shape = predictor(base_img, d)
            Ali = np.array([(shape.part(i).x, shape.part(i).y) for i in range(shape.num_parts)])
            for ali in Ali:
                cv.circle(base_img, (ali[0], ali[1]), 2, (0, 0, 255))

        affine_similarity = True
        Ali_transformed =  Ali @ affine_transform( Ali ,Y , affine_similarity)

        Ali_difference = Ali_transformed - mean1

        k_param = 16

        # alpha = np.linalg.lstsq(U[:, 0:k_param], Ali_difference.ravel(), rcond=None)[0]
        U_limited = U[:, 0:k_param]
        alpha = np.linalg.lstsq( U_limited ,Ali_difference.ravel(), rcond=None)[0]

        print(alpha)

        final = mean1
        for value in range(k_param):
            final = final + alpha[value] * (U[:, value].reshape((68, 2)))
        plot_face(plt, final, edges, color='r')
        # plt.show()

        plot_face(plt, Ali, edges, color='g')
        # plt.show()
        # #
