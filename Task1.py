...
input_img = cv.imread('Camera Roll/31.jpg', 0)
...
for k, d in enumerate(dets2):
    ...
    for x in X:
        cv.circle(input_img, (x[0], x[1]), 2, (250, 220,220))
...
for k, d in enumerate(dets1):
    ...
    for y in Y:
        cv.circle(target_img, (y[0], y[1]), 2, (220, 250, 220))
...
mean = (Y + X_transformed) / 2


plot_face(plt, X, edges, color='b')
plot_face(plt, Y, edges, color='r')
plot_face(plt, X_transformed, edges, color='g')
plot_face(plt, mean, edges, color='y')
